...
- 120 分钟：5 混乱挑战＋40 WHAT／3D 与案例＋10 WHY＋20 公共案例＋40“马上用到我的游戏”＋5 收束。
- S02-A 的汽车／飞机／机器人按钮可追踪同一类预制构件；S02-B 的三层按钮可观察基础零件、功能组件和完整作品；S02-C 可在小车与瞄准观察道具两个课堂案例之间切换，并逐层观察材料方块、塑形组件和功能对象。选择会在教师端与投屏端同步，并随播放会话恢复。
- S02-A/B/C 使用原创几何模型，不下载或热链商业品牌 GIF／贴图：第一景让同一组轮子、透明件、转轴在三种课堂示意间平滑重组；第二景把有限基础件装成齿轮传动、传送带与感应门；第三景把橡胶／铁／塑料或玻璃／金属方块塑形成组件，再组合成对象。
- S02-C 中“《我的世界》式方块”是可编辑粒度的课堂类比，不复刻游戏配方或官方素材；案例二只展示抽象课堂道具的模块层级，不提供现实武器尺寸、结构或使用方法。
- Three.js `0.180.0` 以压缩版随课件本地自托管，许可证见 `vendor/THREE-LICENSE.txt`。WebGL 不可用、关闭脚本或打印时自动保留 HTML/CSS 降级图和完整文字层级。
- 投屏端和教师端都使用课件专属的黄色 `M` 像素图标，并带版本参数避免继续命中旧的全站 `W` 图标缓存；它不替换主站自己的站点图标。
...
- `deck-data.js`：学生可见的 25 页内容真值（对应 S01—S16，S02 拆为 A/B/C）。
...
- `printables/`：速填卡、模块地图、六字段设计卡、正常／异常走查记录，均有 HTML 与 Markdown 初稿。
...
三组场景统一使用本地 Three.js r180 OrbitControls：
- 鼠标单击拖动：上下左右旋转；滚轮：缩放。
- Shift＋拖动或右键拖动：平移；触屏单指旋转、双指缩放／平移。
- 模型内的 `＋`、`−`、`复位`；聚焦模型后方向键旋转，加减号缩放，`0` 复位。
- 俯仰角和远近设有限制，避免模型翻到地板下或无限缩小。
