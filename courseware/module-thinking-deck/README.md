...
T-122 建立的双视图课件，已由 T-128／T-130 更新为 `2026.09.19-r5`：16 个源教学单元对应 18 张 16:9 投屏页面。S02 用三页递进讲清“预制组件重组 → 用基础零件创造组件 → 用材料方块塑形组件并组合对象”，并加入本地自托管的 Three.js 三维课堂示意。
...
- `deck-data.js`：学生可见的 18 页内容真值（对应 S01—S16，S02 拆为 A/B/C）。
