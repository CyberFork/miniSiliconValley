...
T-122 建立的双视图课件，已由 T-128 更新为 `2026.09.18-r4`：16 个源教学单元对应 17 张 16:9 投屏页面，S02 用两页递进讲清“预制组件重组 → 用基础零件创造组件 → 组合整体”，并加入本地自托管的 Three.js 三维课堂示意。
...
- 标准版为 60 分钟。
- S02-A 的汽车／飞机／机器人按钮可追踪同一类预制构件；S02-B 的三层按钮可观察基础零件、功能组件和完整作品。选择会在教师端与投屏端同步，并随播放会话恢复。
- S02-A/B 使用原创几何模型，不下载或热链商业品牌 GIF：前者让同一组轮子、透明件、转轴在三种课堂示意间平滑重组；后者把有限基础件装成齿轮传动、传送带与感应门，第 3 步启动自动搬运。
...
- `deck-data.js`：学生可见的 17 页内容真值（对应 S01—S16，S02 拆为 A/B）。
