...
test("T-119 keeps eleven topics, gates part two, requires part one and removes image uploads", () => {
  ...
  assert.equal((definition.match(/kind: "image"/g) ?? []).length, 0); assert.equal((definition.match(/kind: "table"/g) ?? []).length, 3); assert.equal((definition.match(/required: true/g) ?? []).length, 21);
  assert.match(client, /localStorage\.setItem/); assert.match(client, /required aria-required="true"/); assert.match(client, /styles\.requiredMark/); assert.match(client, /FIRST_GAME_REQUIRED_FIELDS/); assert.match(client, /完成 01—05 后解锁/); assert.match(store, /HOMEWORK_REQUIRED_MISSING/); assert.match(store, /boundedString\(raw\.respondentNickname, "姓名／昵称", 80, true\)/);
