...
test("T-119 keeps ten topics, gates part two, requires part one and removes image uploads", () => {
  ...
  for (const title of ["我的游戏是什么", "谁来玩我的游戏", "游戏怎么玩", "怎样算赢", "画出我的游戏世界", "角色故事", "关卡和任务", "敌人、障碍和道具", "奖励和成长", "胜利、失败和结局"]) assert.match(definition, new RegExp(title));
  ...
  for (const removed of ["growthPath", "玩家怎样从“小菜鸟”变得越来越厉害", "laterDifficulty", "后面的关卡比前面难在哪里", "finalEnding", "游戏最后，玩家会", "11｜游戏画风、颜色和声音", "overallStyle", "mainColors", "characterStyle", "sceneStyle", "victorySound", "failureSound"]) assert.doesNotMatch(definition, new RegExp(removed));
  assert.match(client, /FIRST_GAME_HOMEWORK_SECTIONS\.length/);
