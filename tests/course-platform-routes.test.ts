...
test("T-122 publishes a second D-mentor deck without replacing the classroom default and protects teacher notes", () => {
  ...
  assert.match(runtime, /revision: 4,[\s\S]*?isCurrent: false,[\s\S]*?revision: 5,[\s\S]*?isCurrent: true,[\s\S]*?entryPath: "\/courseware\/development-mentor-module-thinking\/r5\/audience\/"/);
  ...
  assert.match(navigation, /development-mentor-module-thinking\/r5\/teacher\/presenter\.html/);
