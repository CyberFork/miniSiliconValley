...
export default async function FirstGameSubmissionPage({ params }: { params: Promise<{ submissionId: string }> }) {
  ...
  return <main className={styles.page} id="top">
    ...
    <div className={styles.detailMeta}><span>提交时间：{formatTime(submission.createdAt)}</span><span>{submission.answeredCount} 项有内容</span><span>{submission.imageCount} 张图片</span><span>独立记录，不覆盖旧版本</span></div>
    ...
    {submission.answeredCount === 0 && <section className={styles.emptyList}><h2>这是一份空白提交</h2><p>本作业允许部分填写和空白提交，因此这份独立记录仍然有效。</p></section>}
...
function AnswerCard({ field, answer }: { field: FirstGameField; answer: FirstGameAnswer }) {
  if (field.kind === "image" && typeof answer === "string") return <article className={styles.answerCard} data-wide="true"><b>{field.label}</b><Image src={answer} width={1200} height={900} alt={field.label} unoptimized /></article>;
