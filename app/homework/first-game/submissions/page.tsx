...
export default async function FirstGameSubmissionsPage({ searchParams }: { searchParams: Promise<{ page?: string | string[] }> }) {
  ...
  return <main className={styles.page}>
    <header className={styles.top}><BrandHomeLink title="HOMEWORK ARCHIVE" subtitle="公开提交查看" /><nav><a href={publicPath("/homework/first-game/")}>填写一份新作业</a><a href={publicPath("/terminal/")}>时空终端</a></nav></header>
    ...
    <aside className={styles.publicWarning}><strong>公开边界</strong><p>这里按用户明确要求免登录查看。自填昵称不是平台账号身份；本页不展示账号、课堂、钱包、私密卡或其他系统数据。</p></aside>
    {result.submissions.length ? <section className={styles.submissionGrid}>{result.submissions.map((item) => <a className={styles.submissionCard} href={publicPath(`/homework/first-game/submissions/${encodeURIComponent(item.id)}/`)} key={item.id}><small>{formatTime(item.createdAt)}</small><h2>{item.respondentNickname || "匿名 Young Builder"}</h2><p>{item.respondentNote || "没有填写补充说明。"}</p><div><span>{item.answeredCount} 项有内容</span><span>{item.imageCount} 张图片</span></div><b>打开完整作业 →</b></a>)}</section> : <section className={styles.emptyList}><h2>还没有提交</h2><p>这里不会预置假数据。第一份真实提交保存后会出现在这里。</p><a className={styles.backLink} href={publicPath("/homework/first-game/")}>填写第一份作业 →</a></section>}
