...
export default async function ClassroomSeatPage({ params }: { params: Promise<{ classroomId: string }> }) {
  ...
  return <ClassroomRuntime classroomId={classroomId} view="seat" user={{ userId: user.userId, username: user.username, displayName: user.displayName, role: user.role ?? "learner", impersonation: user.impersonation }} />;
}
