...
人工复核截图确认：正式 PNG 本身已有透明通道，但共享 React／静态主题曾人为给图片增加 `#fff` 背景、1px 边框和内边距，导致左上角看起来像未抠图的白色卡片。该表现不是素材问题，而是样式回归。

- 已按官网页脚与同事课件的直接叠加方式，移除 React、公开静态页和 Studio Editor 独立主题中的合成白底、边框和内边距；favicon、课程与课堂真值未改。
- r6 上线后的保护性复验在 Studio Editor 中捕获到一份独立主题仍保留白底，因此没有将 r6 当作最终完成；r7 补齐该副本并把它纳入回归契约。
- 最终 Hecate release：`20260914T102054CST-transparent-wordmark-r7`；运行源码：`189e7f942d409d4417be38051ecf2985143d8b01`；前一版本 r6 可原子回滚。
- 生产公开 8 类路由 × 4 视窗，以及登录后的 10 个应用路由，均实际加载 r7。图片计算样式为透明背景、0 边框、0 内边距；无横向溢出，键盘返回主页仍通过。
- `npm test` 55/55、平台 163/163、部署 53/53、品牌契约 7/7；Hecate health 与 public smoke 通过。Studio Editor 页头另有生产截图。

机器回执和浏览器证据已经更新为 r7；本回执不代签课程内容的人工 View/UI 验收。
