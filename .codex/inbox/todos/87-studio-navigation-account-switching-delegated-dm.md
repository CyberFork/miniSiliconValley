...
> **2026-09-10 用户澄清：本段旧方案已被 [T-106 已登录账号列表与多账号会话切换](106-multi-account-session-switcher.md) 取代。** 用户需要保留本浏览器已登录账号并直接选择切换，而不是先退出再登录。以下步骤保留作历史原因记录，不再作为后续实现要求；T-087 旧完成状态不代表 T-106 已完成。
