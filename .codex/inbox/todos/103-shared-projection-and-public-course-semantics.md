...
- [ ] 修改共享发牌/任务规则只需改一处源码，生成产物可重建。
- [ ] 当前T-095全部195正向场景保持通过，并补导师/中控等未覆盖字段。
- [ ] D名称、5步与独立6分钟Demo、默认人数和可支持人数表述一致。
- [ ] README与匿名/mentor/learner课件权限实测一致。
- [ ] 公开投影不暴露内部或未审内容；历史世界线仍不可被运行状态改写。
- [ ] 完成会影响投影的重构后按T-102更新兼容契约与回执验证。
