...
- **T-093：范围漏项且曾提前关闭。** 旧回执只验证 D 导师 PPT；现已重开原单，Classroom 真实进度条完成本地实现与隔离浏览器验证，仍等待本轮生产发布和线上复验。
- **T-111：删除曾被弱化为可选。** 现已恢复为必交付项；活跃/历史 TEST 的删除 UI、API、权限、原子数据库删除和证据阻断已完成本地实现与隔离浏览器验证，仍等待本轮生产发布。归档、重置没有被当作删除证据。
...
- [ ] 与用户同步生产发布身份、点击验收路径和剩余人工项；代码或局部测试不等同上线。
