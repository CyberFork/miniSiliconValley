...
用户再次指出线上仍显示方形 W 后，确认 r3 只完成了主页链接契约、没有使用正式字标。T-082 再次重开并已在 `20260912T190236CST-official-wordmark-r4` 完成纠正：公开 6 类路由 × 4 视窗、登录后 10 个应用路由均显示同一个蓝绿 `MINI硅谷` 字标，favicon 只作浏览器元数据；生产无溢出、无页面错误、无资源失败。该复核不改变 T-097 其余人工硬件验收边界。
