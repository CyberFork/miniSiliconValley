...
- T-102：已完成并推送。
- T-110：首轮及 T-109 后 Chromium 总回归均完成并推送；生产 r4 公共入口无页面／console／资源错误。真实 Safari／iPad 与硬件上下文操作仍由 T-088 跟踪，故保持 `in-progress`。
- T-108：已完成并推送。
- T-111：已完成并推送；同 exact／跨 revision 多 Test、显眼新建入口、完整身份卡、原子只读归档、回执历史化和真实编译 UI 浏览器回归均通过。
- T-106／T-107：已完成并推送；服务端浏览器账号集合、A／B 直接切换、退出当前／全部、跨标签身份守卫、官网账号入口和响应式可读性均通过。
- T-105：已完成并推送；Studio 验收摘要按课堂授权过滤并脱敏，开放注册准入策略固定，旧课堂 API 在应用层永久 410。
- T-103：已完成工程实现与本地回归；投影规则收敛为服务端直接引用、浏览器构建生成的一份源码，195 场全角色矩阵通过，对外语义及 Released-only 公共摘要收口。兼容契约升级至 `course-projector-v7`，未代签人工回执。下一项为 T-104。
- T-104：已完成工程实现与隔离浏览器回归；课程 exact 审核与家长 QA 缺口保持两个真值，追加事件、显式重开、写盘降级和 server-only loopback 代理闭环；Parent-QA 已纳入同一 Hecate release 与回滚边界。未代签人工回执；下一项为 T-109。
- T-109：已完成并以 `20260911T100357CST-public-ia-t109-r4` 部署到 Hecate；官网／World／教学入口／Studio／Workshop 归档、历史预览数据、框架水合与公共路由全部通过线上只读验收。
- 当前工程实现队列只剩 T-088 的真实 iPad Safari／Android 触摸设备人工验收；T-097 在该证据完成前保持总审计 `in-progress`。
