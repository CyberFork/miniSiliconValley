...
```text
找问题 → 识别真问题 → 想解决方案 → MVP 原型 + VC → 运营 + 品牌 → Demo Day
```

- “课程大纲”的纵向六步跨企业训练同一种能力。
- “企业全流程”沿同一产品横向观察六步闭环。
- 历史 RPG 关卡可以嵌入任一步，为学员提供调查、判断、反馈和迁移练习。
- 项目六步不是单关的“入场—调查—决策—反馈—对照—现实任务”六幕；前者描述数天或数周的项目进程，后者描述一节课内部的学习循环。
- 详见 [课程大纲与内容归集指南](CURRICULUM_OUTLINE.md)。
