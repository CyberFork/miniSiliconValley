> 当前政策（T-105）：公开注册始终创建 active learner，仅可浏览 Released 课件；不会自动获得 Classroom membership，也没有 Studio、Candidate 预览或 Test 身份模拟权限。正式课堂由 Admin DM 分配 membership。旧 `/api/classroom/*` 已退休（返回 410），操作仅针对 `/api/platform/classrooms`。

# 终极管理员／DM 导师主持人操作手册（T-106／T-107）

> 本手册逐项对应当前账号列表与课堂界面。完整发布链与异常处理见 [Course Platform 操作 SOP](COURSE_PLATFORM_SOP.md)。
...
3. 在右上角账户菜单执行“切换账号”：菜单直接列出这台浏览器已验证的账号，点状态为“可切换”的目标账号即可换身份；只有“添加账号”或“需要重新登录”才输入目标账号密码，失败不会退出当前账号。
4. “退出当前账号”只移除当前账号，其他已验证账号仍可选择；“退出此设备全部账号”会再次确认并撤销整组会话。两者都不能靠浏览器后退恢复权限。
5. 从 Studio 切到无 Studio 权限的学员时，系统会转到课堂中心并说明原因；其他标签页会同步身份，未保存编辑先确认，不可把旧草稿以新账号提交。
6. 测试身份模拟时，全站持续显示黄色 `TEST 身份模拟` 横幅。完成查看后必须点击“返回管理员身份”，不能把模拟身份当作真实登录账号继续工作，也不能把它加入真实账号列表。
...
6. 自行注册的学员把完整用户名或昵称告诉你后，在“查找自行注册的账号”中做 exact 查找。系统不会显示全站模糊目录；查到只代表加入下方候选列表，仍需在创建表单中明确分配学员 Membership。
...
- `CONTROLLER_VERSION_CONFLICT`（409 版本冲突）：刷新中控，不重复点击。
